...
1. **Cuts the motor** — pulls the FarDriver **`BL`** (yellow/green) to ground. `Brake` =
   `0-StopWhenGround` (§5).
2. **Lights the brake lamp** — a P-channel MOSFET switches +12 V to the tail's STOP feed. No firmware
   in the path, so the brake light still works if the ESP32 module hangs (plan **D23**).
...
```
 ...
 +12 V ── Q1 source      Q1 drain ── F1 1 A ── tail STOP (red) ── lamp ── common (B−)
          (AO3407A, P-channel)
...
`BL`, Q1's gate and IN-05 are each pulled to ~0.6 V through their own diode → motor cut, lamp on,
module sees it. Lever released → each line returns to its own pull-up; the diodes block current
between them.
...
```
 ACC+ 5.1 V (throttle supply) ── R4 10 kΩ ──┬── RED (pod) ──o/ o── BLUE (pod ground) ── B−
                                            │                 run = closed
                                      R5 100 Ω
                                            │
...
- The pull-up runs from the **throttle's `ACC+`**, not the module, so the toggle works with no module
  fitted. It costs 0.5 mA from a 5.1 V supply that already feeds the throttle.
...
- The module senses the same node on **IN-11** through a **10 k / 15 k divider** (3.0 V at 5 V in) —
  sense only; it never drives the cut.
...
| R1 | 1 | 10 kΩ ¼ W | Q1 gate pull-up to +12 V; also the lever's wetting current (~1.2 mA) |
...
| R4 R5 R6 | 3 | 10 kΩ · 100 Ω · 100 kΩ | Toggle pull-up from `ACC+` · Q2 gate series · Q2 gate-to-ground |
| C1 | 1 | 100 nF ceramic | `BL` ↔ B− at the controller (EMI) |
| F1 | 1 | 1 A automotive mini-blade (ATM) + inline holder | Lamp feed. The STOP lamp draws 0.12 A (M5) |
| — | — | 2-pin locking connectors, 22 AWG, heat-shrink | lever in · `BL` out · lamp out |
...
output**. Check each lever before building (`revv1-inputs-bench-session.md` sitting B):
...
| **2** | When the module's 12 V rail is in | Add Q1, R1, D2L / D2R and F1 → brake lamp | §7.3 |
| **3** | When the module is in | Add D3L / D3R → IN-05 / IN-06 with R3L / R3R | §7.4 |

From step 2 the diodes, Q1 and R1 move onto a small junction board in the module enclosure (plan
board G). Nothing wired at an earlier step changes.
...
| Lamp never lights | F1 blown; D2 reversed; Q1 backwards (its source goes to +12 V); no 12 V rail yet |
